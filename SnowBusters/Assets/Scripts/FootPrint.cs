...
namespace UnityStandardAssets.Characters.ThirdPerson
{
    public class FootPrint : MonoBehaviour
    {
        ...
        void OnCollisionStay(Collision collision)
        {
            ...
            if (collision.gameObject.GetComponent<PlayerInput>().GetMove().sqrMagnitude == 0) return;
            //if (Input.GetAxisRaw("Horizontal") * Input.GetAxisRaw("Horizontal") <= 0.01f && Input.GetAxisRaw("Vertical") * Input.GetAxisRaw("Vertical") <= 0.01f) return;
            if (collision.contacts == null) return;
            if (collision.contacts.Length <= 0) return;
            //Debug.Log(collision.gameObject.name);
            ...
            for (int y = 0; y <= footmark.GetUpperBound(0); y++)
            {
                ...
                {
                    //Debug.LogFormat("[{0}, {1}] = {2}", y, x, footmark[y, x]);
                    AccumulateSnowAdd(tx + x - halfx, ty + y - halfy, -footmark[y, x] / 2);
                }
            }
            // テクスチャー更新
            DisplacementTexture.Apply();
